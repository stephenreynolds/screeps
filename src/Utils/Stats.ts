...
export class Stats
{
    public static build(kernel: Kernel)
    {
        if (!Memory.stats)
        {
            Memory.stats = {};
        ...
        Memory.stats["processes.counts.missed"] =
            (Object.keys(kernel.processTable).length - kernel.execOrder.length - kernel.suspendCount);
        ...
        _.forEach(kernel.execOrder, (execed: { type: string, cpu: number, faulted: boolean }) =>
        {
