...
function DEFCON0() {
  ...
  if (RoomData.tower !== undefined) {
    const structure = _.filter(RoomData.structures, (s: Structure) => {
      return (s.structureType === STRUCTURE_SPAWN && s.hits < s.hitsMax) ||
              ((s.structureType === STRUCTURE_WALL || s.structureType === STRUCTURE_RAMPART) && s.hits < 20000);
    })[0];
    RoomData.tower.repair(structure);
  }
}
...
function DEFCON1(hostiles: Creep[]) {
  ...
  if (RoomData.tower !== undefined) {
    RoomData.tower.attack(hostiles[0]);
  }
}
