...
export class RoomDataProcess extends Process
{
    ...
    public run()
    {
        ...
        if (this.scheduler.data.roomData[this.metaData.roomName].spawns.length === 0)
        {
            ...
            {
                this.scheduler.addProcess(SpawnRemoteBuilderProcess, "srm-" + this.metaData.roomName, 90, {
        ...
        {
            this.scheduler.addProcessIfNotExist(RoomLayoutManagementProcess, "room-layout-" + room.name, 20, {
                roomName: room.name
            });
        }
        ...
        if (this.scheduler.data.roomData[this.metaData.roomName].links.length >= 2)
        {
            this.scheduler.addProcessIfNotExist(LinkProcess, `link-${room.name}`, 30, {
                roomName: room.name
            });
        }
    ...
    public enemyDetection(room: Room)
    {
        ...
        if (enemies.length > 0)
        {
            this.scheduler.addProcessIfNotExist(DefenseManagementProcess, `dm-${this.metaData.roomName}`, 80, {
                roomName: this.metaData.roomName
            });
        }
