...
export class RoomDataProcess extends Process
{
    ...
    public run()
    {
        ...
        if (this.kernel.data.roomData[this.metaData.roomName].spawns.length === 0)
        {
            ...
            const spawnSites = _.filter(this.roomData().constructionSites, (site) =>
            {
    ...
    public build(room: Room)
    {
        ...
        const sourceContainers = _.filter(containers, (container) =>
        {
        ...
        const generalContainers = _.filter(containers, (container) =>
        {
            const matchContainers = [].concat(sourceContainers as never[]) as StructureContainer[];

            const matched = _.filter(matchContainers, (mc) =>
            {
        ...
        const roads = _.filter(structures, (structure) =>
        {
            return (structure.structureType === STRUCTURE_ROAD);
        }) as StructureRoad[];

        const labs = _.filter(myStructures, (structure) =>
        {
            return (structure.structureType === STRUCTURE_LAB);
        }) as StructureLab[];

        const ramparts = _.filter(myStructures, (structure) =>
        {
            return (structure.structureType === STRUCTURE_RAMPART);
        }) as StructureRampart[];

        const links = _.filter(structures, (structure) =>
        {
        ...
        const roomData: RoomData = {
            ...
            extensions: _.filter(myStructures, (structure) =>
            {
                return (structure.structureType === STRUCTURE_EXTENSION);
            }) as StructureExtension[],
            extractor: _.filter(myStructures, (structure) =>
            {
            ...
            ramparts: ramparts,
            roads: roads,
            spawns: _.filter(myStructures, (structure) =>
            {
            ...
            towers: _.filter(myStructures, (structure) =>
            {
        ...
        _.forEach(this.mapFields, (field) =>
        {
        ...
        _.forEach(this.singleFields, (field) =>
        {
